...
//
// NL FURNITURE additions on top of the generated source:
//  - `colors` prop so the palette can follow the admin-configured primary-*
//    theme instead of the hardcoded recipe (see hexToShaderColor below).
//    Changing it re-uploads the uniform; the WebGL context is not rebuilt.
//  - `prefers-reduced-motion` freezes time and disables cursor tracking.
//  - `timeScale` slowed down (see UNIFORMS).
...
const UNIFORMS = {
  ...
  cursorRadius: 0.460,
  oklab: 1.0,
  // Slowed from the generated recipe's -0.537 for a calmer hero.
  timeScale: -0.2,
}

/** The shader takes 0..1 RGB triplets; theme shades are `#rrggbb` strings. */
export type ShaderColor = [number, number, number]

export function hexToShaderColor(hex: string): ShaderColor {
...
/** Pad/truncate to the fixed `vec3 u_colors[8]` uniform array. */
function buildPalette(colors?: ShaderColor[]) {
  const source = colors && colors.length > 0 ? colors.slice(0, MAX_COLORS) : UNIFORMS.colors
  const count = Math.min(source.length, MAX_COLORS)
  const flat = new Float32Array(MAX_COLORS * 3)
  for (let i = 0; i < MAX_COLORS; i++) {
    const [r, g, b] = source[Math.min(i, count - 1)]
    flat[i * 3] = r
    flat[i * 3 + 1] = g
    flat[i * 3 + 2] = b
  }
  return { flat, count: colors && colors.length > 0 ? count : UNIFORMS.colorCount }
}
...
export function ShaderBackground({
  ...
  className?: string
  /** Ramp the silk mixes through, dark → light. Defaults to the recipe colours. */
  colors?: ShaderColor[]
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)

  const colorKey = colors ? colors.map((c) => c.join(",")).join("|") : ""
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const palette = useMemo(() => buildPalette(colors), [colorKey])
  const paletteRef = useRef(palette)
  paletteRef.current = palette
  // Set by the WebGL effect so palette changes re-upload the uniform instead of
  // tearing down and rebuilding the context.
  const pushPalette = useRef<(() => void) | null>(null)
  ...
  useEffect(() => {
    ...
    gl.uniform3fv(uni.colors, paletteRef.current.flat)
    gl.uniform4f(
      uni.shape,
      UNIFORMS.scale,
      UNIFORMS.intensity,
    ...
    // Arrow consts rather than hoisted declarations: TypeScript only keeps the
    // `canvas`/`gl` non-null narrowing inside closures created after the guard.
    const requestRender = () => {
  ...
  // Theme changes (admin picks another primary colour) only re-upload uniforms.
  useEffect(() => {
    pushPalette.current?.()
  }, [palette])
